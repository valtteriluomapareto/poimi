...
struct AppSettingsTests {
    ...
    @Test("denied and restricted deliberately share one 'Off' display; distinct states don't collide")
    func groupingAndUniqueness() {
        func display(_ status: LibraryAuthorization) -> PhotosAccessDisplay { .forAuthorization(status) }
        // Guards the intentional `case .denied, .restricted:` grouping — a future re-split fails here.
        #expect(display(.restricted) == display(.denied))
        // The other states are genuinely distinct (no accidental copy-paste collision).
        #expect(display(.authorized) != display(.limited))
        #expect(display(.authorized) != display(.notDetermined))
        #expect(display(.limited) != display(.denied))
    }
}
