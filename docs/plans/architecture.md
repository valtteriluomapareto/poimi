...
**Adaptive navigation (iOS + iPadOS).** Compact width uses the typed `NavigationStack` above with the zoom push. Regular width (iPad) uses a **2-column `NavigationSplitView`** (#42): the **sidebar** is the album library; the **detail column hosts its own `NavigationStack`** carrying the album's flow — overview → review grid → export. The sidebar's selection binds to `AppCoordinator.activeAlbumID` (the album at the path root), so picking an album swaps the detail and highlights the row; the coordinator's typed path (§11) is therefore *one logical path expressed in two containers*, not literally one stack on iPad. The photo viewer stays a **Now-Playing sheet** over the detail (D10/#36) — so there is no third "photo detail" column (this supersedes an earlier "sidebar · grid · detail" 3-column note that predated the sheet-viewer decision). The review grid's column count is **derived from the detail's width** (`ReviewGridColumns.ideal`, ~132pt cells) so it opens dense on iPad (~6) and reflows on a Split View / Stage Manager resize; pinch overrides within a size class. v1 ships this layout; iPad input polish (keyboard/hover/drag-and-drop) is v1.1.
...
- **Empty + error states (#40, built).** Every non-`.ready` scan phase is actionable / recoverable — never a blank dead-end. `CandidateStore.Phase` carries the *reason*: `.empty(.noPhotosInRange)` (widen the range) vs `.empty(.allExcluded)` (relax the exclusions), and `.failed(.loadError)` (transient — retry) vs `.failed(.accessLost)` (authorization revoked mid-session — re-read auth so the coordinator's root phase flips to `.recovery`, §10, rather than a retry that can't succeed). The states render once in shared views (`ReviewEmptyView` / `ReviewLoadFailedView` / `ReviewAccessLostView`, design 2JE) used by both `ScanningView` and the Overview; the empty copy is the pure, tested `ReviewEmptyCopy`. Export failures (incl. partial) are handled in `ExportView` (#39): per-error messages, notAuthorized→Settings, albumMissing→create-new, and a "N couldn't be added" note when a first export resolves fewer than picked.
...
- **No-`captureDate` assets need a home.** `AssetRef.captureDate` is `Date?` (§2). As shipped they go straight into a fixed **"Undated"** pseudo-section with its own stable key, so they can be reviewed, marked done, and counted like any other. The first draft proposed falling back to the asset's *modification-date* day; that was dropped — the modification date is the **edit** timestamp, so the fallback would have filed a photo under the day it was last cropped and quietly mixed edit-ordering into a capture-ordered timeline. Ordering and grouping read the capture timestamp (`PHAsset.creationDate` → `AssetRef.captureDate`) and nothing else.
