...
- **No-`captureDate` assets need a home.** `AssetRef.captureDate` is `Date?` (§2). They go into a fixed **"Undated"** pseudo-section with its own stable key, so they can be reviewed, marked done, and counted like any other. Ordering and grouping read the capture timestamp (`PHAsset.creationDate` → `AssetRef.captureDate`) and nothing else — D32's original modification-day fallback was dropped before implementation (the modification date is the *edit* timestamp; see the amended rider in [plan-review-decisions.md](./plan-review-decisions.md) D32).
