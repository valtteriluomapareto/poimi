...
- **Ratify D32 = (d)** — unanimous across the 4-review panel; riders folded into architecture.md §13. Author to confirm.
- **v1 build-scope cut for the album library (D31) & mark-as-done (D32).** Pragmatist recommends v1 = a **single active project + derived resume** (defer the albums-list CRUD UI, the completion-stats screen, and auto-complete to v1.1); Architect would ship-and-tighten. The model stays plural-ready either way — this is a build-order call for the author.
