...
- **D4 — Defer location bucketing + named pins to v1.1.** A whole subsystem; the month + total loop works without it. *(Pragmatist §3.)*
  - **↳ #213 Layer 1 (2026-07) — SHIPPED post-v1 (#130); no longer deferred.** DBSCAN place clustering + reverse-geocoded trip/place names landed, default-on per album (`CurationProject.locationEnabled`), names cached in `GeocodedPlaceName` (the D18 pattern). See `App/PoimiApp/Location/` + Curation `PlaceCluster`/`TripOverlay`/`ReviewTimeline`. (The #201/#208 *day-cluster caption* refinement was reverted — PRs #210/#212 — but the underlying subsystem stayed.)
...
- **D32 — Section completion + resume; persist at day granularity (d). ✅ Decided.** Adaptive day-groups are a *computed view*, so persisted "done" needs a stable key. The 4-review panel (Architect, Tester, Pragmatist, Codex) was **unanimous on (d)** — persist per calendar-day, derive section-done — over (a) span / (b) anchor (lose progress on merge/split) and (c) content-hash (breaks on any membership change). Required riders, now in [architecture.md](./architecture.md) §13: a **timezone-stable `DayKey`** shared verbatim with the grouping function (DST-safe via `Calendar.dateInterval`); a defined **home for no-`captureDate` assets** (~~modification-day fallback, else~~ an "Undated" section — **rider amended:** the modification-day fallback was dropped before implementation. `modificationDate` is the *edit* timestamp, so the fallback would have filed a photo under the day it was last cropped, mixing edit-ordering into a capture-ordered timeline; undated assets go straight to "Undated"); a **"done-but-changed" reconcile** (re-open a done day when its membership grows, so a new photo isn't silently swallowed); **resume derived** from `resumeDayKey` (`lastViewedAssetID` is a scroll anchor only); **explicit-tap only in v1** (auto-complete + per-asset "viewed" → v1.1 with `DayProgress`); and **well-defined stats** (denominator = assets on done days, so %-kept can't exceed 100; label it "marked done"). *(Design round + 4-persona/Codex review.)*
- **D33 — v1 sections stay date-only; location/trip labeling remains v1.1 (reinforces D4).** The design's "by-location" overview and trip names are an **additive view** over the same date day-groups once location bucketing lands — not a rework. The only real choice is whether to pull a lightweight EXIF trip-detection into v1; **recommendation: no** — keep v1 scope to chronological day-groups. **Forward-compat invariant (Tester):** when location-aware grouping lands, the "done-state invariant under regrouping" test (D32(d)) must still pass — a trip is just another view over the same per-day truth, never a parallel done-key. *(Design round; reinforces D4; 4-persona/Codex review.)*
  - **↳ #213 Layer 1 (2026-07) — the additive-view architecture SHIPPED (#130), exactly as predicted:** trips relabel the same date day-groups (`ReviewTimeline` is a view over the per-day truth) and the forward-compat invariant held (`Completion.reopening`, D38, unchanged). Only the *scope* note is historical now — trip/place labeling is no longer "v1.1", and the "no EXIF trip-detection in v1" recommendation was superseded.
...
- **D35 — The review grid is an ACCORDION (one cluster open at a time), superseding done-driven collapse.** Device-testing the idea-③ "mark done → collapse" model led to a better one: exactly one day-group cluster is expanded (its full grid) at a time; every other is a collapsed peek. "Done" is **decoupled** from collapse — its own state (a green seal badge) set by a **"Mark as done" button at the END of an open cluster's photos** (discoverable once you've reviewed the day), which collapses that cluster and **advances to the next unreviewed** one. Opening a cluster collapses the previous; initial open = first-unreviewed (soft resume). Only the open cluster loads full-res cells (the rest are 56pt peek thumbs) — a real perf bound. Still on date day-groups (D33). *(User direction, device-validated; shipped #89.)*
- **D36 — Scroll positioning uses iOS-18 `ScrollPosition` (one-shot `scrollTo`), never a maintained `.scrollPosition(id:)` binding.** The two-way `.scrollPosition(id:)` binding writes the top item back and RE-APPLIES it on any re-layout — snapping the grid on select-all / mark-done (#81/#82, regressed twice). `ScrollPosition` with an imperative `scrollTo` doesn't re-apply, so the grid stays put; the #37 month-drill targets a real cell id (a pinned section header isn't a resolvable anchor — why the header-targeted drill silently no-op'd on device). The materializing `ScrollViewReader.scrollTo` stays avoided at scale. *(Recurring #81/#82 lesson; shipped #89.)*
  - **↳ #232 (2026-08) — sanctioned exception: a PAGER whose position IS the state.** The review grid's
    horizontal cluster pager (`ReviewGridView`) keeps a maintained `.scrollPosition(id: $currentPageID)`
    binding: for a pager, re-applying the page on re-layout is the *desired* behavior (the page is the
    selection), so the D36 failure mode doesn't exist there — same unwritten exception the filmstrip
    already carried. Nil scroll-writes are filtered before they reach the state (a nil falling back to
    `clusters.first` would mis-target the top bar's tappable done seal). The ban stands unchanged for
    the *vertical* grids, where the top item is incidental, not semantic.
- **D37 — Additive-optional SwiftData fields ride automatic lightweight migration; no declared stage.** `AppSchemaV1` is a single shared model class, so an explicit `MigrationStage` for an additive *optional* attribute isn't even expressible without freezing an old model copy — and isn't needed (SwiftData auto-adds the nil column). The version stamp stays `1.0.0`; a declared v2 + stage is required only for a **non-lightweight** change (rename / type change / new entity). Verified by installing OVER a prior on-device build (the in-memory test tier can't exercise it). Fulfills project-phases task-9's "establish a migration approach." *(Architect review of #89; also documented in `AppSchema.swift`.)*
  - **↳ #213 Layer 1 (2026-07) — corrected: a NEW ENTITY also rides automatic lightweight migration.** #130 added `GeocodedPlaceName` (a new entity) and advanced the stamp to **`AppSchemaV2` (2.0.0)** — still under *automatic* migration (no `SchemaMigrationPlan`), disproving "new entity ⇒ staged plan." The real staged-plan trigger is a **rename / type-change / data-transform**; and staging is in fact *impossible* while entities share one model class (the version-hash mismatch → "unknown model version"), so a staged plan would first require freezing per-version model copies. Authoritative policy now lives in the `AppSchema.swift` header.
- **D38 — The "done-but-changed" reconcile (the D32(d) rider) is implemented via a per-day candidate snapshot.** `CurationProject.reviewedIDsByDay` (a `[DayKey → ids]` JSON baseline, `.externalStorage`) records the candidate set at each load; on the next load `Completion.reopening` re-opens any done day that **gained** an id, so the collapse never hides a new unreviewed photo. The first load (no baseline) reopens nothing; an empty or corrupt baseline is treated as no-baseline — never a whole-year re-open. *(Architect + Codex review of #89.)*
...
- Burst/hidden/shared-library asset handling — note as open, decide during the slice. *(Within-overlay swipe navigation shipped in v1 — #180/#181 — so it moved to "Resolved" below.)*
...
- ~~**Degraded `.limited` mode vs hard-gate** (D6 leans hard-gate) — settle when the onboarding /
  authorization flow is built (#31).~~ **Resolved (#31, #213 Layer 1):** the onboarding/authorization flow shipped (`OnboardingView` + `AccessRecoveryView`) — hard-gate with `.limited`/denied routed to recovery. No longer open.
