...
| **Warning** — amber (heads-up) | `Color.brandWarning` (`#FF9500` / `#FF9F0A`, asset `BrandWarning`) | **Over-target / ahead-of-pace orientation (#170):** the unclamped count past target ("+N over"), the amber ring/bar segment past the target tick, and the "picking ahead of pace" projection. A *heads-up*, **never** enforcement and **never** the destructive red. A saturated orange, deliberately distinct from the desaturated tan-gold accent (they co-appear on the Overview). |
