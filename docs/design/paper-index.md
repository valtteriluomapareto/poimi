...
- **File:** "Poimi" · page "Page 1" (`1-0`) · **62 artboards** (product screens incl. the #187/#191
  post-export/forward-path designs + 2 pacing design artboards + a 6-artboard v1.1 idea-backlog cluster)
...
| **Review grid · review complete** ✓ | `4SU-0` | 390×844 | **Design for #187 (forward path).** When every day-cluster is marked done, a Liquid-Glass **completion bar** floats over the grid (replacing the page/select-all pills): green check · "All photos reviewed · 1,847 photos · 148 picks" · a gold **Save to Photos** pill — so the grid offers a clear next step instead of `advance()` silently holding position. Built by cloning `4AB-0`. | #187 · **design proposed** |
...
| **Photo viewer · video page** ✓ | `4HY-0` | 390×844 | **Design for #125/#177 (include videos).** A video candidate renders as its **poster frame + a centred glass tap-to-play control** (plays inline; tap to pause; no scrubber) + a Photos-style **duration badge** (bottom-left "▶ 0:14"). The #127 chrome is intact — date · time + "N/target" tally + ⓘ (whose panel adds duration) + the Picked pill; the filmstrip badges video thumbs. On a video page, tap = play/pause (tap-to-pick disabled; Pick via the button, the #127 gesture lesson). | #125/#177 · **design proposed** |
| **Photo viewer · bottom v2** ⬦ | `4MF-0` | 390×844 | **Bottom-area tweak (proposed).** **Apple-Photos-style date typography** — regular weight, spelled month + year + clear time ("Sat, 11 Jul 2026 · 17:15"), fixing the cramped locale form ("ma 21.4. · 11.05"; in-app uses the system long-date + short-time so FI renders "…klo 17.15"). The photo's info (resolution; device + size later) is a **small always-on muted line** ("3024 × 4032 · 12 MP") — the **ⓘ toggle + glass panel are removed**. The two **counts group on the right**: gold pick tally over the "12 of 53" position (moved off the left info line). Transport (‹ Pick ›) + filmstrip unchanged. Built from `4JD-0`. | #183 · **built** |
| **Review · end-of-set card** ✓ | `4QU-0` | 390×844 | **Design for #187 (forward path).** The viewer's end-of-set card (reached by Pick/Next past the last photo, #180) gains a **primary "Save to Photos"** finish action: a gold "reviewed everything" check emblem + "You've reached the end · 148 picks across 12 days", then **Save to Photos** (gold) / **Keep reviewing** (outline) / **Back to grid** (text), bottom-anchored for thumb reach. Consumes #185's Photos-qualified label. Built by cloning `4MF-0`. | #187 · **design proposed** |
| **Photo viewer · pick + auto-advance** ⬦ | `4JD-0` | 390×844 | **Viewer redesign for #180 (one-tap triage).** Centre **Pick** is a reversible toggle (hollow ring + "Pick" → fills gold + "Picked"): picks + **auto-advances** on an unpicked photo, un-picks in place on a picked one. **‹ / › + swipe are pure navigation** ("Skip" = Next; there's no per-photo reject state). Chrome (album-art card, date·time + tally + ⓘ) + filmstrip unchanged. Reshaped by an HIG + UX review (Skip records nothing → honest Next; fire-and-advance broke un-pick → reversible toggle; + labels, debounce, "Picked · Undo" toast, end-of-set card, a11y). Built from `4FE-0`. | #180 · **signed off → building** |
...
| **Albums · post-export status** ✓ | `4O8-0` | **Design for #191 (status honesty + drift).** The album library (`21T-0`) with two post-export row states: **Exported** (green check · "Exported · 200 in Photos", renamed from "Done" for past-tense honesty) and a NEW **Edited since export** (amber `#FF9500` up-arrow-in-circle · "Edited since export · 3 to add") — a distinct glyph + the heads-up tone (distinct from green-done + gold-in-progress) for when picks changed since the last export. Overview drift hint (near "Update in Photos") to follow. | #191 · **design proposed** |
