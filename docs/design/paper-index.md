...
- **File:** "Poimi" · page "Page 1" (`1-0`) · **43 artboards** (36 product screens + a 6-artboard
...
| **Review grid · header + ring** ✓ | `4AB-0` | 390×844 | **Redesign variant C — current direction (awaiting sign-off, 2026-07-08).** The user's re-split of B: **cluster identity moves into a solid top bar** — back chevron + gold pin + cluster name ("Käynti kohteessa Nokia") + "47 kuvaa" count on the leading lane; a **compact circular progress ring** (gold arc on a track) with "147 / 300" picked/total on the trailing lane, *replacing the linear progress bar*. **Only per-page controls float as glass pills over the grid**: a "3 / 12" **page-number pill** (pages icon) leading + a **select-all glass icon** trailing. Photos edge-to-edge (gapless 3-up, gold-check selection). **Open Q: no Export/Vie in the grid top** — export would move to the Overview; pending confirmation. | #35 (grid-top) · **design proposed (variant C — preferred)** |
