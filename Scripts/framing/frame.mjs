...
//
//  Usage:  node frame.mjs <raw.png> <out.png> "<line 1>" "<line 2>"
//
...
// iPhone 17 Pro Max frame (frames/iphone-17-pro-max.png, 1470×3000) + screen rect from offsets.json
// ("iPhone 17 Pro Max": offset "+75+66", width 1320 → our 1320×2868 screenshot fits exactly).
const FRAME = {
	png: path.join(DIR, 'frames/iphone-17-pro-max.png'),
	w: 1470,
	h: 3000,
	screen: { x: 75, y: 66, w: 1320, h: 2868 },
};

// App Store 6.9" output canvas.
const CANVAS = { w: 1320, h: 2868 };
// Phone placement on the canvas.
const PHONE_W = 1012;
const PHONE_Y = 566;
...
if (!fs.existsSync(FRAME.png)) {
	console.error(`missing device bezel ${FRAME.png} — run Scripts/framing/fetch-frames.sh first`);
	process.exit(1);
}

const roundRect = (w, h, r, fill) =>
	Buffer.from(
		`<svg width="${w}" height="${h}"><rect width="${w}" height="${h}" rx="${r}" ry="${r}" fill="${fill}"/></svg>`,
	);

// 1. Framed phone: screenshot into the frame's screen cutout, bezel PNG on top (its transparent,
//    rounded screen area reveals the screenshot; the opaque bezel masks the square corners).
//    Round the screenshot corners to the screen radius first, so its square corners don't poke into
//    the device's rounded-off (transparent) outer corners.
const SCREEN_R = 152;
...
// 2. Scale the framed phone down to sit below the headline.
...
// 3. Branded background + hero headline (Helvetica placeholder → embed Inter in the real pipeline).
const bg = Buffer.from(`<svg width="${CANVAS.w}" height="${CANVAS.h}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${CANVAS.w / 2}" y="270" text-anchor="middle" font-family="Helvetica" font-size="86" font-weight="800" fill="#1C1C1E" letter-spacing="-2">${line1}</text>
  <text x="${CANVAS.w / 2}" y="386" text-anchor="middle" font-family="Helvetica" font-size="86" font-weight="800" fill="#1C1C1E" letter-spacing="-2">${line2}</text>
  <rect x="${CANVAS.w / 2 - 72}" y="456" width="144" height="9" rx="4.5" fill="#D08A2A"/>
</svg>`);

// 4. Soft drop shadow shaped like the phone body (rounded rect), blurred + offset.
const shadowR = Math.round(PHONE_W * 0.155);
const shadow = await sharp(roundRect(PHONE_W, phoneH, shadowR, 'rgba(20,22,28,0.28)'))
	.extend({ top: 70, bottom: 70, left: 70, right: 70, background: { r: 0, g: 0, b: 0, alpha: 0 } })
	.blur(48)
	.png()
	.toBuffer();

// 5. Compose, flatten to opaque (App Store rejects alpha), write.
await sharp(bg)
	.composite([
